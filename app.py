...
def add_member():
	...
	member_curr = db.execute('select id,name,level,email from members where name = ?', [name])
	member_fetch = member_curr.fetchone()

	return jsonify({'member' : {'id':member_fetch['id'], 'name':member_fetch['name'], 'email':member_fetch['email'], 'level':member_fetch['level']}})
